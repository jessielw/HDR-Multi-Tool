...
def root_exit_function():  # Asks if user wants to close main GUI + close all tasks
    ...
    if confirm_exit:
        try:
            subprocess.Popen(
                f"TASKKILL /F /im hdr10plus_tool.exe /T",
                creationflags=subprocess.CREATE_NO_WINDOW,
            )
            subprocess.Popen(
                f"TASKKILL /F /im dovi_tool.exe /T",
                creationflags=subprocess.CREATE_NO_WINDOW,
            )
            root.destroy()
        except (Exception,):
...
root.title("HDR-Multi-Tool-Gui v1.37")
...
try:
    ...
    messagebox.showinfo(
        title="Error",
        message="Could Not Write to config.ini file, delete and try again",
    )
...
def set_hdr10plus_tool_path():
    ...
    path = filedialog.askopenfilename(
        title='Select Location to "hdr10plus_tool.exe"',
        initialdir="/",
...
options_menu.add_command(
    label='Set "hdr10plus_tool.exe" path', command=set_hdr10plus_tool_path
)
...
def set_dolbyvision_tool_path():
    ...
    path = filedialog.askopenfilename(
        title='Select Location to "dovi_tool.exe"',
        initialdir="/",
...
options_menu.add_command(
    label='Set "dovi_tool.exe" path', command=set_dolbyvision_tool_path
)
...
if not pathlib.Path(
    ...
    if shutil.which("ffmpeg") is not None:
        ...
        error_prompt = messagebox.askyesno(
            title="Error!",
            message="Cannot find ffmpeg, " 'please navigate to "ffmpeg.exe"',
        )
        if not error_prompt:
            messagebox.showerror(
                title="Error!", message="Program requires ffmpeg.exe to work correctly"
            )
            root.destroy()
        if error_prompt:
            set_ffmpeg_path()
...
if not pathlib.Path(
    ...
    if pathlib.Path("apps/HDR10PlusTool/hdr10plus_tool.exe").is_file():
        ...
        hdr10plus_tool = (
            '"' + str(pathlib.Path("apps/HDR10PlusTool/hdr10plus_tool.exe")) + '"'
        )
        try:
            config.set("hdr10plus_parser_path", "path", hdr10plus_tool)
            with open(config_file, "w") as configfile:
        ...
        messagebox.showerror(
            title="Error!",
            message="Please download hdr10plus_tool.exe and set path to "
...
if not pathlib.Path(dolbyvision_tool.replace('"', "")).is_file():
    if pathlib.Path("apps/dovi_tool/dovi_tool.exe").is_file():
        ...
        messagebox.showerror(
            title="Error!",
            message="Please download dovi_tool.exe and set path to "
...
link_input_label = Label(
    info_frame,
    text="Please Open a Dolby Vision or HDR10+ compatible file",
    background="#434547",
    foreground="white",
    height=1,
    font=("Helvetica", 10),
...
hdr_tool_tabs.grid(
    row=2, column=0, columnspan=4, sticky=E + W + N + S, padx=16, pady=(5, 0)
)
...
def hdr10plus_parse():
    ...
    if (
        dolbyvision_parse.get() == "off" and hdr_parse.get() == "off"
    ):  # If both boxes un-checked, disable start
        start_button.configure(state=DISABLED)


hdr_parse = StringVar()
hdr_parse_checkbox = Checkbutton(
    hdr_frame,
    text="Parse HDR10+ Metadata",
    variable=hdr_parse,
    onvalue="on",
    offvalue="off",
    command=hdr10plus_parse,
    takefocus=False,
)
hdr_parse_checkbox.grid(
    row=0, column=0, columnspan=1, rowspan=1, padx=10, pady=6, sticky=N + W
)
hdr_parse_checkbox.configure(
    background="#434547",
    foreground="white",
...
dolbyvision_parse_checkbox = Checkbutton(
    dolby_frame,
    text="Parse Dolby Vision RPU",
    variable=dolbyvision_parse,
    onvalue="on",
    offvalue="off",
    command=dolby_parse,
    takefocus=False,
)
dolbyvision_parse_checkbox.grid(
    row=0, column=0, columnspan=1, rowspan=1, padx=10, pady=6, sticky=N + W
)
dolbyvision_parse_checkbox.configure(
    background="#434547",
    foreground="white",
    activebackground="#434547",
    activeforeground="white",
...
dolbyvision_crop_checkbox = Checkbutton(
    dolby_frame,
    text="Check if cropping encode",
    variable=dolbyvision_crop,
    onvalue=" -c",
    offvalue="",
    takefocus=False,
)
dolbyvision_crop_checkbox.grid(
    row=0, column=1, columnspan=2, rowspan=1, padx=10, pady=6, sticky=N + E
)
dolbyvision_crop_checkbox.configure(
    background="#434547",
    foreground="white",
    activebackground="#434547",
    activeforeground="white",
...
dobly_vision_mode_choices = {
    "Extract RPU: Untouched": "-m 0",
    "Extract RPU: MEL": "-m 1",
...
dobly_vision_mode_menu_label = Label(
    dolby_frame, text="Parsing Mode :", background="#434547", foreground="white"
)
dobly_vision_mode_menu_label.grid(
    row=1, column=0, columnspan=1, padx=10, pady=0, sticky=W + S
)
dobly_vision_mode_menu = OptionMenu(
    dolby_frame, dobly_vision_mode, *dobly_vision_mode_choices.keys()
)
dobly_vision_mode_menu.config(
    background="#23272A", foreground="white", highlightthickness=1, width=18, anchor=W
)
dobly_vision_mode_menu.grid(
    row=2, column=0, columnspan=1, padx=10, pady=(5, 5), sticky=W + N
)
...
start_code_mode_choices = {
    "Default": "",
    "Four": "--start-code four",
    "Annex-b": "--start-code annex-b",
}
start_code_menu_label = Label(
    dolby_frame,
    text="         HEVC NALU Start Code :",
    background="#434547",
    foreground="white",
)
start_code_menu_label.grid(row=1, column=1, columnspan=1, padx=10, pady=0, sticky=S)
start_code_menu = OptionMenu(
    dolby_frame, start_code_mode, *start_code_mode_choices.keys()
)
start_code_menu.config(
    background="#23272A", foreground="white", highlightthickness=1, width=18, anchor=W
)
...
def check_for_hdr():  # Block of code to check for what type of HDR there is
    try:
        mediainfo_hdr_parse = MediaInfo.parse(
            VideoInputQuoted.replace('"', "")
        )  # Remove quotes from input
        for track in mediainfo_hdr_parse.tracks:
            if (
                track.track_type == "Video"
            ):  # Set's mediainfo to parse video portion of the input file
                try:
                    hdr_format_string = "".join(track.other_hdr_format)
                    if (
                        "Dolby Vision" in hdr_format_string
                        and "HDR10+" in hdr_format_string
                    ):
                        messagebox.showinfo(
                            title="Information",  # Source is Dolby Vision and HDR10+
                        ...
                        link_input_label.config(
                            text=hdr_format_string.rstrip("\n"), anchor=W
                        )
                        Hovertip(
                            link_input_label,
                            hdr_format_string.rstrip("\n"),
                            hover_delay=600,
                        )
                        ...
                        "Dolby Vision" in hdr_format_string
                    ):  # If source only has Dolby Vision
                        link_input_label.config(
                            text=hdr_format_string.rstrip("\n"), anchor=W
                        )
                        hdr_tool_tabs.select(dolby_frame)
                        ...
                        messagebox.showinfo(
                            title="Info",
                            message="Input has no HDR10+ or Dolby Vision metadata, "
                            "no parsing needed",
                        )
                        link_input_label.config(
                            text="Input has no HDR10+ or Dolby Vision metadata, "
                            "no parsing needed",
                            anchor=CENTER,
                        )
                        hdr_tool_tabs.select(hdr_frame)
                        ...
                        Hovertip(
                            link_input_label,
                            "Input has no HDR10+ or Dolby Vision, " "no parsing needed",
                            hover_delay=600,
                        )
                    ...
                    if hdr_format_string is None:
                        messagebox.showinfo(
                            title="Info",
                            message="Input has no HDR metadata, no parsing needed",
                        )
                        link_input_label.config(
                            text="Input has no HDR metadata, no parsing needed",
                            anchor=CENTER,
                        )
                        hdr_tool_tabs.select(hdr_frame)
                        hdr_parse.set("off")
                        ...
                        Hovertip(
                            link_input_label,
                            "Input has no HDR metadata, no parsing needed",
                            hover_delay=600,
                        )
    except (Exception,):
        input_error = messagebox.askyesno(
            title="Error",
            message="Could not automatically detect HDR type, "
...
def input_button_commands():
    ...
    VideoInput = filedialog.askopenfilename(
        ...
        title="Select A File",
        filetypes=(
            ("MKV, MP4, HEVC, TS", "*.mkv *.mp4 *.hevc *.ts"),
            ("All Files", "*.*"),
        ),
    )
    input_entry.configure(state=NORMAL)
    ...
    if VideoInput:
        if file_extension in supported_extensions:
            ...
            if file_extension == ".hevc":  # If input file is *.hevc show message
                messagebox.showinfo(
                    title="Info",
                    message="Mediainfo does not currently support parsing a raw HEVC "
            ...
            for track in mediainfo_hdr_parse.tracks:
                if track.track_type == "General":
                    if track.count_of_video_streams is not None:
                        ...
                        start_button.configure(state=DISABLED)
                        messagebox.showinfo(
                            title="Info", message="Input has no video streams"
                        )
        else:
            link_input_label.config(
                text="Please Open a Dolby Vision or HDR10+ compatible file",
...
def update_file_input(*args):
    ...
    if file_extension in supported_extensions:
        ...
        for track in mediainfo_hdr_parse.tracks:
            if track.track_type == "General":
                if (
                    track.count_of_video_streams is not None
        ...
        link_input_label.config(
            text="Please Open a Dolby Vision or HDR10+ compatible file", anchor="center"
        )
        messagebox.showinfo(
            title="Wrong File Type",
            message="Try Again With a Supported File Type!\n\nIf this is a "
...
def file_save():
    global VideoOutput
    VideoOutput_window = filedialog.asksaveasfilename(
        defaultextension="",
...
def start_job():
    ...
    if shell_options.get() == "Default":
        try:
            ...
            total_duration = str(int(total_file_size) / 1000).rsplit(".", 1)[
                0
            ]  # Compressed code for progress bars
        except (Exception,):
            pass
        ...
        def close_encode():
            confirm_exit = messagebox.askyesno(
                title="Prompt",
                message="Are you sure you want to stop the parser?",
                parent=window,
            )
            if confirm_exit:
                try:
        ...
        encode_window_progress.grid(
            row=1, column=0, pady=(10, 6), padx=10, sticky=E + W
        )
    ...
    if dolbyvision_parse.get() == "on":
        if shell_options.get() == "Default":
            finalcommand = (
                ...
                + dolbyvision_tool
                + " "
                + dobly_vision_mode_choices[dobly_vision_mode.get()]
                + dolbyvision_crop.get()
                + start_code_mode_choices[start_code_mode.get()]
                + " extract-rpu - -o "
                + str(VideoOutputQuoted)
                + '"'
            )
...
start_button = HoverButton(
    root,
    text="Start Job",
    command=check_for_existing_output,
    foreground="white",
    background="#23272A",
    borderwidth="3",
    activebackground="grey",
